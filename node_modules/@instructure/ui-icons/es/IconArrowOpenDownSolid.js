var _path;

import React, { Component } from 'react';
import { SVGIcon } from '@instructure/ui-svg-images';

class IconArrowOpenDownSolid extends Component {
  constructor() {
    super(...arguments);
    this.ref = null;

    this.handleRef = el => {
      const elementRef = this.props.elementRef;
      this.ref = el;

      if (typeof elementRef === 'function') {
        elementRef(el);
      }
    };
  }

  render() {
    return /*#__PURE__*/React.createElement(SVGIcon, Object.assign({}, this.props, {
      name: "IconArrowOpenDown",
      viewBox: "0 0 1920 1920",
      elementRef: this.handleRef
    }), _path || (_path = /*#__PURE__*/React.createElement("path", {
      d: "m.08 568.063 176.13-176.13 783.988 783.864 783.74-783.864 176.129 176.13-959.87 960.118z",
      fillRule: "evenodd"
    })));
  }

}

IconArrowOpenDownSolid.glyphName = 'arrow-open-down';
IconArrowOpenDownSolid.variant = 'Solid';
IconArrowOpenDownSolid.displayName = 'IconArrowOpenDownSolid';
IconArrowOpenDownSolid.propTypes = { ...SVGIcon.propTypes
};
IconArrowOpenDownSolid.allowedProps = [...SVGIcon.allowedProps];
export default IconArrowOpenDownSolid;
export { IconArrowOpenDownSolid };